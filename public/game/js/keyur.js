(function (window, document) {
  ...
  window.keyurJS = {
    ...
    getOns: function () {
      webJS.socket.on("get-trade-harvest", that.getTradeHarvest);
      webJS.socket.on("trade-accept", that.tradeAcceptedBySubTribe);
      webJS.socket.on("goods-expired", that.goodsExpired);
      webJS.socket.on("bounty-hunter-completion", that.getCompletionRate);
      webJS.socket.on("updated-users", that.updateUsers);
      webJS.socket.on("transfer-credits", that.getCreditsFromSubTribes);
    },
    ...
    showTransferCredits: function () {
      ...
      html +=
        ...
        '<div style="height: 5%"></div>' +
        '<div class="input-form" style="height: 11%;">' +
        '<input class="input-text" type="tel" id="txt_credits" placeholder="Cronos" style="width: 80%; height: 100%;"/>' +
        "</div>" +
        '<div style="height: 5%"></div>' +
        '<div class="input-form">Remaining Cronos : ' +
        webJS.credits +
        "</div>" +
        '<div style="height: 5%"></div>' +
        '<div class="button" id="btn-transfer-credits">' +
        '<div class="table">' +
    ...
    transferCredits: function () {
      ...
      if (friend == "") {
        ...
        webJS.emitContent("server-transfer-credits", param);
    ...
    sendCreditsToBank: function () {
      ...
      if (isNaN(credits)) {
        ...
        webJS.emitContent("server-submit-credits", params);
        that.manageCredits("sub", credits);
    ...
    sendMVoBank: function () {
      ...
      if (mv < 0) {
        ...
        webJS.emitContent("server-submit-mv", params);
    ...
    acceptTrade: function () {
      ...
      if (that.harvestTrade[trade][harvest]["status"] == "pending") {
        ...
        if (parseInt(price) <= webJS.credits && price > -1) {
          ...
          webJS.emitContent("server-trade-accepted", params);
    ...
    tradeAcceptedBySubTribe: function (data) {
      ...
      webJS.emitContent("server-goods-expired", params);
    },
    // declining the trade
    ...
    sellItToFriend: function () {
      ...
      if (price >= 0 && friend != "") {
        if (
          ...
          webJS.totalHarvest[index]["status"] = "on sale";
          webJS.emitContent("trade-good-with-friends", param);
          document.getElementById("trading-popup").style.display = "none";
        } else {
    ...
    bountySubmitMV: function () {
      if (that.myMov >= 2) {
        ...
        webJS.emitContent("server-submit-mv-to-bounty-hunter", params);
    ...
    showCrobs: function () {
      var html =
        ...
        '<div class="bank-container">' +
        '<div style="height: 7%"></div>' +
        '<div class="button" id="submit-credits">' +
        '<div class="table">' +
        '<div class="cell font-14">' +
        "Send Cronos" +
        "</div>" +
